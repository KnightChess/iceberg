...
public class TestSparkMetadataColumns extends SparkTestBase {
  ...
  public void testPartitionMetadataColumnWithManyColumns() {
    ...
    final List<String> colKeys =
        IntStream.range(1, 1010).mapToObj(i -> "c" + i).collect(Collectors.toList());
    final List<Column> colValues =
        IntStream.range(1, 1010)
            .mapToObj(i -> expr("CAST(id as STRING)"))
            .collect(Collectors.toList());
    ...
    Dataset<Row> df =
        spark
            .range(2)
            .withColumns(
                JavaConverters.asScalaBuffer(colKeys), JavaConverters.asScalaBuffer(colValues));
